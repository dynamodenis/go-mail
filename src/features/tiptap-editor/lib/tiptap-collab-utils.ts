...
export const USE_JWT_TOKEN_API_ENDPOINT = false;
 ...
 */
export const fetchCollabToken = async (TIPTAP_COLLAB_TOKEN: string) => {
  ...
  // Tokens should come from useCurrentUser() which fetches them from the backend API.
  // The backend generates JWT tokens for each authenticated user.
  if (!TIPTAP_COLLAB_TOKEN) {
    console.warn(
      "Tiptap collaboration token not available. Ensure user is authenticated and useCurrentUser() returns tiptap_collab_jwt."
    );
  }
 ...
 */
export const fetchAiToken = async (TIPTAP_AI_TOKEN: string) => {
  if (USE_JWT_TOKEN_API_ENDPOINT) {
    try {
      // Example API endpoint that returns a JWT token.
      // TODO: implement this API endpoint in your app
  ...
  if (!TIPTAP_AI_TOKEN) {
    console.warn(
      "Tiptap AI token not available. Ensure user is authenticated and useCurrentUser() returns tiptap_ai_jwt."
    );
  }
